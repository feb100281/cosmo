# corporate/reports/excel_report/sheets/sheet_root_categories.py
...
def build_root_categories_sheet(wb, years, rows):
    ws = wb.create_sheet("Root Categories")
    ...
    if is_last_year_incomplete:
        ...
        note = "Показана выручка по группам категорий в разрезе лет, изменение к предыдущему году и итог за период."

    # -------------------------------------------------
    # Layout колонок
    # -------------------------------------------------
    layout = [("name", "Группа категорий")]
    ...
    draw_sheet_header(
        ...
        subtitle="Выручка по группам категорий в разрезе лет",
        note=note,
    ...
    for r in rows:
        root_category = r.get("root_category") or "—"
        year = int(r.get("year"))
        full_amount = float(r.get("full_amount") or 0)
        ytd_amount = float(r.get("ytd_amount") or 0)

        data[root_category][year] = {
            "full_amount": full_amount,
            "ytd_amount": ytd_amount,
        }

    def get_display_value(root_name, year):
        payload = data[root_name].get(year, {})
