...
def build_index_sheet(wb, generated_at=None):
    ...
    links = [
        ...
        ("6", "Категории", "Выручка категорий в разрезе лет", "key"),
         ("7", "Подкатегории", "Выручка подкатегорий в разрезе лет", "key"),
        ("8", "Группы категорий", "Сводка по укрупненным группам", "key"),
        ("9", "Зависимость категории от производителя", "Концентрация выручки по производителям", "key"),
        ("10", "Матрица производитель × категория", "Перекрестный анализ производителей и категорий", "key"),
        ("11", "Ширина ассортимента по производителям", "Активные SKU и эффективность по производителям", "key"),
        ("12", "Ширина SKU по подкатегориям", "Активные SKU и эффективность по категориям / подкатегориям", "key"),
        ("13", "Эффективность SKU", "Проверка, компенсируется ли изменение SKU ростом выручки на 1 SKU", "key"),
        ("14", "Новые и потерянные SKU", "Динамика ассортимента: новые и выбывшие товары по производителям", "key"),
        ("15", "Спрос по ценовым диапазонам", "Показывает, какие диапазоны цены реально формируют спрос и выручку внутри подкатегорий", "key"),
        ("16", "Ассортимент vs выручка", "Сопоставление ассортимента и результата", "key"),
        ("17", "ABC по производителям", "ABC-анализ производителей", "key"),
        ("18", "ABC по категориям", "ABC-анализ категорий", "key"),
        ("19", "Магазин × производитель", "Матрица каналов/магазинов и производителей", "key"),
        ("20", "Топ товаров", "Лидеры продаж по товарам", "key"),
