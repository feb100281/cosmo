...
def build_assortment_width_subcategories_sheet(wb, years, rows):
    ws = wb.create_sheet("Ширина SKU по подкатегориям")
    ...
    if is_last_year_incomplete and compare_prev_year is not None:
        note = (
            f"Лист показывает активную ширину ассортимента по категориям и подкатегориям: "
        ...
        note = (
            "Лист показывает активную ширину ассортимента по категориям и подкатегориям: "
    ...
    draw_sheet_header(
        ws,
        title="Ширина SKU по подкатегориям",
        subtitle="Активные SKU и эффективность по категориям / подкатегориям",
        note=note,
        line_to_col=12,
    )
    ...
    def get_period_metrics(name_key, year):
        payload = data[name_key].get(year, {})
    ...
    sorted_names = sorted(
        data.keys(),
        key=lambda name_key: (
            get_period_metrics(name_key, compare_last_year)["amount"] if compare_last_year is not None else 0,
            get_period_metrics(name_key, compare_last_year)["sku_count"] if compare_last_year is not None else 0,
            name_key[0],
            name_key[1],
        ),
        reverse=True,
    ...
    for category, subcategory in sorted_names:
        key_name = (category, subcategory)

        prev_metrics = get_period_metrics(key_name, compare_prev_year) if compare_prev_year is not None else {"sku_count": 0, "qty": 0, "amount": 0}
        last_metrics = get_period_metrics(key_name, compare_last_year) if compare_last_year is not None else {"sku_count": 0, "qty": 0, "amount": 0}
