...
def build_subcategories_sheet(wb, years, rows):
    ...
    if is_last_year_incomplete:
        ...
        note = "Показана выручка по подкатегориям в разрезе лет, изменение к предыдущему году и итог за период."

    # -------------------------------------------------
    # Layout
    # -------------------------------------------------
    layout = [("name", "Категория / Подкатегория")]
    ...
    draw_sheet_header(
        ws,
        title="Подкатегории",
        subtitle="Выручка по подкатегориям в разрезе лет",
        note=note,
    ...
    if not rows or not years:
        style_total_row(
            ws,
            row=data_start_row,
            values=["Нет данных"] + [""] * (len(headers) - 1),
        )
    ...
    def get_display_value(payload_by_year, year):
        payload = payload_by_year.get(year, {})
    ...
    def make_row_values(name, payload_by_year):
        row_values = []
        number_formats = {}
        display_values_by_year = {}

        for y in years:
            display_values_by_year[y] = get_display_value(payload_by_year, y)
        ...
        for key, _label in layout:
            if key == "name":
                ...
                prev_payload = payload_by_year.get(prev_y, {})
                curr_payload = payload_by_year.get(curr_y, {})

                if is_last_year_incomplete and curr_y == last_year:
                    prev_val = float(prev_payload.get("ytd_amount", 0) or 0)
                    curr_val = float(curr_payload.get("ytd_amount", 0) or 0)
                else:
                    prev_val = float(prev_payload.get("full_amount", 0) or 0)
                    curr_val = float(curr_payload.get("full_amount", 0) or 0)
        ...
        return row_values, number_formats, row_total

    def category_total(cat_name):
        total = 0
        for _, payload_by_year in data[cat_name].items():
            for y in years:
                total += get_display_value(payload_by_year, y)
        return total

    sorted_categories = sorted(
        data.keys(),
        key=lambda x: (category_total(x), x),
        reverse=True,
    )
    ...
    for category in sorted_categories:
        category_payload_by_year = {}

        for y in years:
            full_sum = 0
            ytd_sum = 0

            for _, payload in data[category].items():
                full_sum += float(payload.get(y, {}).get("full_amount", 0) or 0)
                ytd_sum += float(payload.get(y, {}).get("ytd_amount", 0) or 0)

            category_payload_by_year[y] = {
                "full_amount": full_sum,
                "ytd_amount": ytd_sum,
            }

            totals_full_by_year[y] += full_sum
            totals_ytd_by_year[y] += ytd_sum

        row_values, number_formats, row_total = make_row_values(category, category_payload_by_year)
        grand_total += row_total

        # Родительская строка категории
        style_data_row(
            ws,
            row=cur_row,
            values=row_values,
        ...
        for col_idx in range(1, last_used_col + 1):
        ...
        category_row = cur_row
        cur_row += 1

        # Дочерние строки подкатегорий
        sorted_subcategories = sorted(
            data[category].keys(),
            key=lambda s: (
                sum(get_display_value(data[category][s], y) for y in years),
                s,
            ),
            reverse=True,
        )

        first_sub_row = cur_row

        for subcategory in sorted_subcategories:
            row_values, number_formats, _ = make_row_values(
                f"    ↳ {subcategory}",
