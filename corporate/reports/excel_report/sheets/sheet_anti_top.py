# corporate/reports/excel_report/sheets/sheet_anti_top.py
...
def build_anti_top_sheet(wb, anti_top_data, anti_top_categories_data=None):
    ws = wb.create_sheet("Anti-top manufacturers")
    ws.sheet_properties.outlinePr.summaryBelow = False

    years_for_display = anti_top_data.get("years_for_display", [])
    rows = anti_top_data.get("rows", [])

    anti_top_categories_data = anti_top_categories_data or {}
    category_rows = anti_top_categories_data.get("rows", [])

    is_ytd_comparison = anti_top_data.get("is_ytd_comparison", False)
    cutoff_year = anti_top_data.get("cutoff_year")
    cutoff_month = anti_top_data.get("cutoff_month")
    cutoff_day = anti_top_data.get("cutoff_day")
    compare_prev_year = anti_top_data.get("compare_prev_year")
    compare_last_year = anti_top_data.get("compare_last_year")
    ...
    if is_ytd_comparison and cutoff_year and cutoff_month and cutoff_day:
        note = (
            f"Показаны производители с наибольшим снижением выручки. "
        ...
        note = "Показаны производители с наибольшим снижением выручки относительно предыдущего года."
    ...
    # отдельный разделитель перед аналитическим блоком
    if years_for_display:
        layout.append(("gap_before_change", ""))

    change_label = "Изменение YTD" if is_ytd_comparison else "Изменение"
    change_pct_label = "Изменение YTD, %" if is_ytd_comparison else "Изменение, %"

    layout.extend(
        [
            ("diff_amount", change_label),
            ("diff_pct", change_pct_label),
            ("decline_share", "Вклад в падение"),
        ]
    )
    ...
    draw_sheet_header(
        ws,
        title="Просадка производителей",
        subtitle="Производители с наибольшим снижением выручки",
        note=note,
    ...
    for row in rows:
        ...
        for key, _label in layout:
            if key == "name":
                ...
                row_values.append(row["decline_share"])
                number_formats[len(row_values)] = FORMATS["pct"]
        ...
        for category_name in category_names:
            ...
            # показываем только категории с падением
            if cat_diff >= 0:
                continue
            ...
            cat_decline_share = 0 if row["diff_amount"] == 0 else abs(cat_diff) / abs(row["diff_amount"])
            ...
            for key, _label in layout:
                if key == "name":
                    ...
                    cat_row_values.append(cat_decline_share)
                    cat_number_formats[len(cat_row_values)] = FORMATS["pct"]
    ...
    for key, _label in layout:
        if key == "name":
            ...
            total_values.append(total_decline_share)
            total_number_formats[len(total_values)] = FORMATS["pct"]
