...
def _fetch_managers_shipments_period(date_from, date_to_excl, big_order_threshold=100_000):
    q = """
        WITH money AS (
            SELECT
        ...
        orders AS (
            SELECT
                ...
                COUNT(*)     AS orders_with_shipments,
        ...
        SELECT
            ...
            money.cr,
            money.quant,
            COALESCE(orders.orders_with_shipments, 0) AS orders_with_shipments,
...
def get_mtd_managers_shipments_bundle(d, big_order_threshold=100_000):
    ...
    return {
        "cur": _fetch_managers_shipments_period(ms, me_excl, big_order_threshold),
        "ly":  _fetch_managers_shipments_period(ms_ly, me_ly_excl, big_order_threshold),
        "pm":  _fetch_managers_shipments_period(ms_pm, me_pm_excl, big_order_threshold),
    }


def get_ytd_managers_shipments_bundle(d, big_order_threshold=100_000):
    d = pd.to_datetime(d).date()
    ...
    return {
        "cur": _fetch_managers_shipments_period(ys, ye_excl, big_order_threshold),
        "ly":  _fetch_managers_shipments_period(ys_ly, ye_ly_excl, big_order_threshold),
    }