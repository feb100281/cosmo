...
#         self.CRITICAL_COLOR = "D32F2F"  # Красный - критично
#         self.HIGH_RISK_COLOR = "F57C00"  # Оранжевый - высокий риск
#         self.MEDIUM_RISK_COLOR = "FBC02D"  # Желтый - средний риск
#         self.LOW_RISK_COLOR = "7CB342"  # Зеленый - низкий риск
...
#         # KPI карточки
#         self._draw_kpi_cards(row, summary_data)
#         row += 6
...
#             if "КРИТИЧНО" in risk:
#                 row_fill = PatternFill("solid", fgColor="FFEBEE")  # Светло-красный фон
#             elif "ВЫСОКИЙ" in risk:
#                 row_fill = PatternFill("solid", fgColor="FFF3E0")  # Светло-оранжевый
#             else:
#                 row_fill = fill
...
#                 order.get('days_without_payment', 0),
...
#                 elif col_idx == 13:  # Оплаты сводка
#                     cell.alignment = Alignment(horizontal="left", vertical="center", wrap_text=True)
#                     cell.font = Font(name="Roboto", size=8)
#                 else:  # Числовые поля
#                     cell.alignment = Alignment(horizontal="right", vertical="center")
#                     cell.font = Font(name="Roboto", size=9)
#                     if isinstance(value, (int, float)) and col_idx not in [10]:
#                         cell.number_format = '#,##0'
...
#         total_row = [
#             "ИТОГО:", "", "", "", "", "",
#             "", total_shipped, "", total_underpayment, "", "", "", ""
#         ]
...
#         # Настройка колонок
#         col_widths = {
#             "B": 28, "C": 12, "D": 22, "E": 28, "F": 20, "G": 22,
#             "H": 16, "I": 16, "J": 14, "K": 14, "L": 12, "M": 25, "N": 14, "O": 30
#         }
...
#                     border_cell.border = BORDERS["thin"]
...
class UnpaidShipmentsSheet(BaseSheet):
    ...
    def build(self, orders_data, summary_data):
        ...
        # ========== ИТОГОВАЯ СТРОКА - ПРОСТОЙ РАБОЧИЙ ВАРИАНТ ==========
        total_shipped = summary_data['total_shipped_amount']
        total_underpayment = summary_data['total_underpayment']
        ...
        # Остальные колонки с суммами
        # Колонка 7 (H) - Сумма заказа
        cell = self.ws.cell(row=row, column=7, value=total_amount_active)
        cell.fill = total_fill
        cell.border = thin_border
        cell.font = Font(name="Roboto", bold=True, size=10)
        cell.alignment = Alignment(horizontal="right", vertical="center")
        ...
        # Колонка 8 (I) - Отгружено всего
        cell = self.ws.cell(row=row, column=8, value=total_shipped)
        cell.fill = total_fill
        cell.border = thin_border
        cell.font = Font(name="Roboto", bold=True, size=10)
        cell.alignment = Alignment(horizontal="right", vertical="center")
        ...
        # Колонка 9 (J) - Оплачено
        cell = self.ws.cell(row=row, column=9, value=total_cash_pmts)
        cell.fill = total_fill
        cell.border = thin_border
        cell.font = Font(name="Roboto", bold=True, size=10)
        cell.alignment = Alignment(horizontal="right", vertical="center")
        ...
        # Колонка 10 (K) - Недоплата
        cell = self.ws.cell(row=row, column=10, value=total_underpayment)
        cell.fill = total_fill
        cell.border = thin_border
        cell.font = Font(name="Roboto", bold=True, size=10)
        cell.alignment = Alignment(horizontal="right", vertical="center")
        ...
        # Заполняем остальные колонки (3,4,5,6,11,12,13,14,15) пустыми с заливкой
        for col in [3, 4, 5, 6, 11, 12, 13, 14, 15]:
